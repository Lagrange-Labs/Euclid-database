...
pub(crate) struct BlockTreeInputs<const MAX_DEPTH: usize> {
...
/// from the public API.
pub(crate) struct BlockTreeCircuitInputs<const MAX_DEPTH: usize> {
    base_inputs: BlockTreeInputs<MAX_DEPTH>,
    previous_proof: ProofWithVK,
}

impl<const MAX_DEPTH: usize> From<BlockTreeCircuitInputs<MAX_DEPTH>>
    for (BlockTreeInputs<MAX_DEPTH>, ProofWithVK)
{
    fn from(val: BlockTreeCircuitInputs<MAX_DEPTH>) -> Self {
        (val.base_inputs, val.previous_proof)
    }
}
