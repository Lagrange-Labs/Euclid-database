...
pub struct Parameters<const BLOCK_DB_DEPTH: usize> {
    /// The regular wires for the logic of RevelationCircuit
    revelation: RevelationWires,
...
pub struct RevelationRecursiveInput {
    /// values expected by the RevelationCircuit main logic
    logic_inputs: RevelationCircuit,
...
impl<const BLOCK_DB_DEPTH: usize> Parameters<BLOCK_DB_DEPTH> {
        ...
        [(); <PoseidonHash as Hasher<F>>::HASH_SIZE]:,
    {
        ...
        let query2_block_pi = block::BlockPublicInputs::<Target>::from(
            query2_block_verifier_wires.get_public_input_targets::<F, QUERY_ERC_BLOCK_NUM_IO>(),
        );

        assert_eq!(query2_block_pi.inputs.len(), QUERY_ERC_BLOCK_NUM_IO);
    ...
    fn generate_proof_internal(
        &self,
        inputs: RevelationRecursiveInput,
    ...
    pub fn generate_proof(&self, inputs: RevelationRecursiveInput) -> Result<Vec<u8>> {
